...
FFmpegInput::open(const std::string& name, ImageSpec& spec)
{
    ...
    if (m_data_stream == -1) {
        // no default data stream was found, use the first one found
        m_data_stream = first_data_stream;
    }
...
FFmpegInput::read_frame(int frame)
{
    ...
    int ret      = 0;
    int eof      = 0;
    while (ret == 0 || m_codec_cap_delay) {
        if (!eof) {
            ret = av_read_frame(m_format_context, &pkt);
            if (ret == AVERROR_EOF) {
                eof = 1;
            }
        }
        if (pkt.stream_index == m_video_stream) {
            ...
            finished = receive_frame(m_codec_context, m_frame, eof ? NULL : &pkt);
